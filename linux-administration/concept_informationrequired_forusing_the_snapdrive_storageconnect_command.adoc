...
`_lun_name_` (forme longue ou courte)
 ...
 a| 
Le `file_spec` remis à `-fs` Est le nom du point de montage du système de fichiers lors de la connexion d'un système de fichiers créé directement sur une LUN.
...
`-nolvm`
 ...
 a| 
Pour connecter un système de fichiers sur un volume hôte : le `-fs _file_spec_` et `-hostvol _file_spec_` Vous indiquez le système de fichiers LVM, le groupe de disques et les volumes hôtes que vous souhaitez connecter à un nouvel hôte.
...
* Le `_-mntopts_` argument est un système de fichiers `-type` option spécifiée à l'aide de `mount` commande `-o` drapeau. Ne pas inclure le `-o` dans le `-mntopts` argument. Par exemple, la séquence -mntopts tmplag passe la chaîne `-otmplog` à la `mount` et insère le texte tmplag sur une nouvelle ligne de commande.
+
