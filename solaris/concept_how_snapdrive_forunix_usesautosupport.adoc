...
[role="lead"]
Les messages AutoSupport sont consignés dans la vue log EMS du système de stockage lorsque `_autosupport-enabled_` l'option est définie sur `on` dans le `snapdrive.conf` fichier. Par défaut, cette option est définie sur `_on_` dans le `snapdrive.conf` fichier. SnapDrive for UNIX envoie des messages AutoSupport au système de stockage, dans lequel les messages sont consignés dans le système de gestion des événements (EMS).
